...
def receive_stock(body: KgReceiveStockRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    try:
...
@router.get("/summary")
def get_summary(branch_id: UUID, kg_item_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
